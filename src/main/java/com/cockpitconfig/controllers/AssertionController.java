...
/*	        if (communicationVia != null) {
	        	commMedium = Integer.parseInt(communicationVia);
	        }*/
	        return mav;
        }
	    ...
	    if (ruleName != null && communicationVia != null) {
	    	...
	    	if (ifExists == -1) {
	        	...
	        	removeExistingTimeConstraints(lastInsertedGroupIndex);	// Before saving new screen remove all rules from timeConstraints Table
       ...
       // If communicationMedium is selected as Email then set email recipnet in communicationViaEmail table.
	 ...
	 * @param recipents E-Mail id of the user to which application should inform upon violation of constraints
	 * @param assertionGrpIndex Primary key of assertiongroup table corresponding to current screen
	 */
	 ...
	 * @param selectedDays Each element of array corresponds to a day, if day is selected then true otherwise false
	 * @param startTime start time(HH:MM format) for constraint to start
	 * @param endTime end time(HH:MM format) for constraint to end
	 * @param assertionGrpIndex index of this constraint among all constraint set for a rule
	 */
	public void setTimeConstraints (String[] selectedDays, String startTime, String endTime, int assertionGrpIndex) {
		...
		for (int i = 0; i < checkedDays.length; ++ i) {
			if(checkedDays[i] == true) {
				switch(i) {
				case 0: break;
				case 1: tc.setMonday(1);	break;
				case 2: tc.setTuesday(1);	break;
				case 3: tc.setWednesday(1);	break;
				case 4: tc.setThursday(1);	break;
				case 5: tc.setFriday(1);	break;
				case 6: tc.setSaturday(1);	break;
				case 7: tc.setSunday(1);	break;
				default: break;
				}
			}
		}
	...
	public int setAssertionGroupID (String ruleName, String communicationVia) {
		...
		return(agDao.getLastInsertedIndex(ag));
	}

	/**
	 * Function which sets all the parameters for a rule
	 * @param CriteriaIndex	 	Index of given Criteria as described in CRITERIA table.
	 * @param isAreIndex	 	1 = is/are, 2 = has slope
	 * @param slopeIndex	 	1 = less than, 2 = equal to, 3 = greater than
	 * @param numberField	 	contains the value of number field
	 * @param TimeFrameIndex 	Index of given TimeFrame as described in TIMEFRAME table.
	 * @param NotificationIndex	Index of given Notification as described in NOTIFICATION table.
	...
	public void setAssertionCondition (int CriteriaIndex, int isAreIndex, int slopeIndex, int numberField, int TimeFrameIndex,
		...
		SqlSessionFactory sf = (SqlSessionFactory)getServletContext().getAttribute("sqlSessionFactory");
    	AssertionConditionDAO criteriaDao = new AssertionConditionDAO(sf);
    	criteriaDao.addNewRule(ac);
	}
    ...
    private void handleFormSubmission(HttpServletRequest request, ModelAndView mav) {
    	...
    	for (int i = 0; i < groupName.size(); ++i) {
    		rulesTemp[i] = groupName.get(i).getConstraintName();
    	}
    	...
    	for (int i = 0; i < criteriaConditions.size(); ++i) {
    		criteriaTemp[i] = criteriaConditions.get(i).getCondition();
    	}
    	mav.addObject("criterias", criteriaTemp);
     ...
     * @param existingRuleName		Name of the rule selected by User
    ...
    private void updateScreen (String existingRuleName, HttpServletResponse response) throws Exception {
    	...
    	int[] existingCriteria = new int[ruleInfo.size()];
    	...
    	for (int i = 0; i < ruleInfo.size(); ++ i) {
			existingCriteria[i] = ruleInfo.get(i).getCriteriaID();
        ...
        jsonResult.put("existingCriteria", existingCriteria);
